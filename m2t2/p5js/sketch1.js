...
function draw() {
  ...
  if (count <= 0) {
      ...
      particle2.show();
      particle1.show();
      document.getElementById('catsong').pause();
    ...
    if (value) {
      ...
      if (data.length === 7) { // Ensure there are 7 values (x1, y1, x2, y2, button1, button2, button3)
        ...
        if (button1Data === 1 && !timerStarted) {
          ...
          var audio = document.getElementById('catsong');
          audio.play();
          console.log('audio played!');
        }

        // Winner message
        if (button2Data === 1 && timerFinished) {
        ...
        if (button3Data === 1 && timerFinished) {
        ...
        // reset game
        if (button1Data === 1 && timerFinished) {
          console.log('reload');
          location.reload();
        }
      }
